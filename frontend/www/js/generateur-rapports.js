document.addEventListener('DOMContentLoaded', async () => {
  ...
  function updatePreview() {
    ...
    if (!preview) return;
    preview.innerHTML = `<div class="grid grid-cols-2 gap-3 mb-5"><div class="rounded-lg bg-white dark:bg-gray-900 p-3"><p class="text-xs text-gray-500">Zone</p><strong>${escapeHtml(region)}</strong><p class="text-xs text-gray-500">${escapeHtml(district)}</p></div><div class="rounded-lg bg-white dark:bg-gray-900 p-3"><p class="text-xs text-gray-500">Sortie</p><strong>${escapeHtml(format)}</strong><p class="text-xs text-gray-500">${indicators.length} indicateur(s)</p></div></div><div class="space-y-2">${indicators.length ? indicators.map((label, index) => `<div class="flex items-center gap-3"><span class="w-32 truncate text-xs">${escapeHtml(label)}</span><div class="h-2 flex-1 rounded-full bg-gray-200 dark:bg-gray-700"><div class="h-2 rounded-full bg-primary transition-all duration-500" style="width:${Math.min(94, 42 + index * 13)}%"></div></div></div>`).join('') : '<p class="text-sm text-gray-500">Sélectionnez au moins un indicateur.</p>'}</div>`;
  }
  ...
  async function loadIndicators() {
    ...
    container.innerHTML = result.length ? result.map((indicator, index) => `<label class="flex items-center gap-3 py-1 cursor-pointer"><input type="checkbox" value="${indicator.id}" data-label="${escapeHtml(indicator.nom)}" ${index < 3 ? 'checked' : ''} class="h-5 w-5 rounded border-gray-300 text-primary"><span>${escapeHtml(indicator.nom)}</span></label>`).join('') : '<p class="text-xs text-gray-500">Aucun indicateur configuré.</p>';
